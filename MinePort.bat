...
:fail
cls
echo Failed to connect to an internet connection
echo -------------------------------------------------
echo we require internet to download java updates and server files
echo -------------------------------------------------
echo please connect to a vaild and working internet connection and try again
echo -------------------------------------------------
...
echo 4) Press 4 to goto BeforeMenu
echo #####################################################
...
if %Selection%==4 goto BeforeMenu
...
if %Selection%==n goto BeforeMenu
goto Reminder
...
cls
curl -s https://api.adoptium.net/v3/info/available_releases > releases.json
for /f "tokens=2 delims=:" %%A in ('findstr /i "most_recent_feature_version" releases.json') do (
    set version=%%A
)
set version=%version: =%
set version=%version:,=%
...
curl --fail --show-error -L -o Java.zip "%zip_url%"
if %errorlevel%==22 goto fail1
if %errorlevel%==0 goto proceed1
...
echo updating :D
echo --------------------------------------------
set "url=https://api.adoptium.net/v3/binary/latest/25/ga/windows/x64/jdk/hotspot/normal/adoptium"
curl -L -o java.zip "%url%"
powershell -Command "Expand-Archive -Path 'java.zip' -DestinationPath 'Java'"
del Java.zip
cd Java
...
echo 3) Press 4 to goto BeforeMenu
set /p Selection="Select:"
...
echo 2) Press 2 To change a server's display name
echo 3) Press 3 to install a server (Installs a server without downloading it) (1 time use per server) (For Minecraft Modding)
echo 4) Press 4 to goto BeforeMenu
set /p Selection="Select:"
